...
const apiMocks = vi.hoisted(() => ({
  ...
  editMessage: vi.fn<(slug: string, messageId: number, content: string) => Promise<{ id: number; content: string; editedAt: string }>>(),
  deleteMessage: vi.fn<(slug: string, messageId: number) => Promise<void>>(),
  addReaction: vi.fn<(slug: string, messageId: number, emoji: string) => Promise<{ messageId: number; emoji: string; userId: number; username: string }>>(),
  removeReaction: vi.fn<(slug: string, messageId: number, emoji: string) => Promise<void>>(),
  searchMessages: vi.fn<(slug: string, query: string) => Promise<SearchResult>>(),
  uploadAttachments: vi.fn<(slug: string, files: File[], options?: { onProgress?: (percent: number) => void; messageContent?: string; replyTo?: number | null; signal?: AbortSignal }) => Promise<UploadResult>>(),
  markRead: vi.fn<(slug: string, messageId?: number) => Promise<ReadStateResult>>(),
  globalSearch: vi.fn<(query: string, params?: { usersLimit?: number; groupsLimit?: number; messagesLimit?: number }) => Promise<GlobalSearchResult>>(),
  getRoomAttachments: vi.fn<(slug: string, params?: { limit?: number; before?: number }) => Promise<RoomAttachmentsResult>>(),
}))
...
describe('ChatController', () => {
  ...
  it('deduplicates in-flight room details by slug', async () => {
    ...
    settle({ slug: 'abc', name: 'Room', kind: 'private', created: false, createdBy: null })
    const [first, second] = await Promise.all([firstPromise, secondPromise])
  ...
  it('delegates pass-through methods to apiService with original args', async () => {
    const directStartResponse: DirectStartResponseDto = {
      slug: 'dm_1',
      kind: 'direct',
      peer: { username: 'alice', profileImage: null, avatarCrop: null, lastSeen: null, bio: null },
    }
    const unreadCounts: UnreadCountItem[] = [{ roomSlug: 'room_1', unreadCount: 3 }]
    const editedMessage = { id: 7, content: 'edited', editedAt: '2026-01-01T00:00:00.000Z' }
    const reaction = { messageId: 7, emoji: '🔥', userId: 2, username: 'alice' }
    const search: SearchResult = {
      results: [{ id: 1, username: 'alice', content: 'hello', createdAt: '2026-01-01T00:00:00.000Z', highlight: 'hello' }],
      pagination: { limit: 20, hasMore: false, nextBefore: null },
    }
    const upload: UploadResult = {
      id: 10,
      content: 'files',
      attachments: [{ id: 1, originalFilename: 'a.txt', contentType: 'text/plain', fileSize: 4, url: null, thumbnailUrl: null, width: null, height: null }],
    }
    const readState: ReadStateResult = { roomSlug: 'room_1', lastReadMessageId: 44 }
    const searchResult: GlobalSearchResult = {
      users: [{ username: 'alice', profileImage: null, avatarCrop: null, lastSeen: null }],
      groups: [{ slug: 'g1', name: 'Group 1', description: '', username: 'group1', memberCount: 2, isPublic: true }],
      messages: [{ id: 9, username: 'alice', content: 'hello', createdAt: '2026-01-01T00:00:00.000Z', roomSlug: 'room_1', roomName: 'Room 1', roomKind: 'group' }],
    }
    const attachments: RoomAttachmentsResult = {
      items: [{ id: 2, originalFilename: 'b.txt', contentType: 'text/plain', fileSize: 2, url: null, thumbnailUrl: null, width: null, height: null, messageId: 22, createdAt: '2026-01-01T00:00:00.000Z', username: 'alice' }],
      pagination: { limit: 20, hasMore: false, nextBefore: null },
    }
    ...
    await expect(chatController.startDirectChat('alice')).resolves.toEqual(directStartResponse)
    await expect(chatController.getUnreadCounts()).resolves.toEqual(unreadCounts)
    await expect(chatController.editMessage('room_1', 7, 'edited')).resolves.toEqual(editedMessage)
    await expect(chatController.deleteMessage('room_1', 7)).resolves.toBeUndefined()
    await expect(chatController.addReaction('room_1', 7, '🔥')).resolves.toEqual(reaction)
    await expect(chatController.removeReaction('room_1', 7, '🔥')).resolves.toBeUndefined()
    await expect(chatController.searchMessages('room_1', 'hello')).resolves.toEqual(search)
    await expect(chatController.uploadAttachments('room_1', [file], { messageContent: 'files' })).resolves.toEqual(upload)
    await expect(chatController.markRead('room_1', 44)).resolves.toEqual(readState)
    await expect(chatController.globalSearch('alice', { usersLimit: 5, groupsLimit: 3, messagesLimit: 2 })).resolves.toEqual(searchResult)
    await expect(chatController.getRoomAttachments('room_1', { limit: 20, before: 100 })).resolves.toEqual(attachments)
    ...
    expect(apiMocks.uploadAttachments).toHaveBeenCalledWith('room_1', [file], { messageContent: 'files' })
    expect(apiMocks.markRead).toHaveBeenCalledWith('room_1', 44)
    expect(apiMocks.globalSearch).toHaveBeenCalledWith('alice', { usersLimit: 5, groupsLimit: 3, messagesLimit: 2 })
    expect(apiMocks.getRoomAttachments).toHaveBeenCalledWith('room_1', { limit: 20, before: 100 })
  })
})
