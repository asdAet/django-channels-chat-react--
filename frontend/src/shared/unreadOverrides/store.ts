 ...
 *
 * Правило слияния намеренно сохраняет локальный `0`, если пришедший с сервера
 * снапшот unread ещё сообщает положительный unread по этой комнате. Это защищает
 * текущий чат от отката бейджа назад, когда локальное чтение уже зафиксировано,
 * а авторитативный push-снимок ещё не догнал только что отправленный `mark_read`.
 *
