...
export function ConversationListProvider({ user, ready, children }: Props) {
  ...
  const [globalResults, setGlobalResults] = useState<GlobalSearchResult>(EMPTY_GLOBAL_RESULTS)
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [counts, groups] = await Promise.all([
        ...
        groupController.getMyGroups().catch(() => ({ items: [] as GroupListItem[], total: 0 })),
      ])
  ...
  return <ConversationListCtx.Provider value={value}>{children}</ConversationListCtx.Provider>
}
