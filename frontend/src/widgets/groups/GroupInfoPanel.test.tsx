  ...
  AvatarCropModal: ({
    ...
    open ? (
      <div data-testid="avatar-crop-modal">
        <button type="button" onClick={() => onApply({ x: 0.1, y: 0.2, width: 0.5, height: 0.5 })}>
          apply-crop
        </button>
...
const withBaseMocks = () => {
  groupControllerMock.getGroupMembers.mockResolvedValue({ items: [], total: 0 })
  ...
  chatControllerMock.getRoomAttachments.mockResolvedValue({ items: [], pagination: { limit: 120, hasMore: false, nextBefore: null } })
  chatControllerMock.getRoomMessages.mockResolvedValue({ messages: [], pagination: { limit: 100, hasMore: false, nextBefore: null } })
}
...
describe('GroupInfoPanel', () => {
  ...
  it('hides self moderation actions in members view', async () => {
    ...
    expect(within(selfRow).queryByRole('button', { name: 'Kick' })).not.toBeInTheDocument()
    expect(within(selfRow).queryByRole('button', { name: 'Ban' })).not.toBeInTheDocument()
    expect(within(selfRow).queryByRole('button', { name: 'Mute' })).not.toBeInTheDocument()

    const otherRow = screen.getByTestId('group-member-2')
    expect(within(otherRow).getByRole('button', { name: 'Kick' })).toBeInTheDocument()
  })
  ...
  it('opens avatar crop modal and sends avatar crop with updateGroup', async () => {
    if (!URL.createObjectURL) {
      ...
      vi.spyOn(URL, 'createObjectURL').mockReturnValue('blob:test-group-avatar')
    }
    ...
    fireEvent.change(screen.getByTestId('group-avatar-input'), { target: { files: [file] } })
  ...
  it('loads media and links sections', async () => {
    ...
    await waitFor(() => {
      expect(chatControllerMock.getRoomAttachments).toHaveBeenCalledWith('test-group', { limit: 120 })
    })
