...
class CanManageRoomRolesTests(SimpleTestCase):
    ...
    def test_denies_when_room_id_missing_or_invalid(self):
        ...
        view_with_invalid_id = SimpleNamespace(kwargs={"room_id": ""})
        self.assertFalse(self.permission.has_permission(request, view_with_invalid_id))

    def test_denies_when_user_not_authenticated(self):
        request = SimpleNamespace(user=SimpleNamespace(is_authenticated=False))
        view = SimpleNamespace(kwargs={"room_id": 1})
    ...
    def test_allows_when_service_grants_access(self, actor_can_manage_roles_mock):
        ...
        actor_can_manage_roles_mock.assert_called_once_with(1, request.user)
