...
class DirectInboxConsumerTests(TransactionTestCase):
    ...
    def test_active_room_incoming_message_is_read_immediately(self):
        """Проверяет, что активная direct-комната не создает unread."""
        async def run():
            """Проверяет сценарий `run`."""
            ...
            self.assertNotIn(str(self.direct_room.pk), inbox_payload['unread']['counts'])
